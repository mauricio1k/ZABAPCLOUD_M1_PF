...
*   Validate and update order.
  ...
  METHOD delete_work_order.
    ...
    DATA(lv_validate_delete) = mo_validate->validate_delete_order( iv_work_order_id = iv_work_order_id
                                                                   iv_status        = iv_status  ).
    ...
    ls_order_log = VALUE ztorderhist_0217(  history_id          = iv_work_order_id && lv_records
                                            ...
                                            modification_date   = cl_abap_context_info=>get_system_date( )
                                            change_description  = |Work order: { iv_work_order_id } has been deleted| ).
